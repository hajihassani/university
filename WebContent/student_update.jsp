...
<title>Update student - University</title>
<script
...
<% if(studentID != null) { %>
	<div class="container">
		<form id="StudentData" method="post" enctype='application/json'>
			...
			<button type="submit" class="btn btn-primary" id="UpdateStudent">Update
				Student</button>
		</form>
	</div>
	...
	<script>
		...
		$(document).ready(function() {
			var url = "api/students/student/" + <%= studentID %>;
			$.get(url, function(data, status) {
				$("#inputId").val(data.id);
				$("#inputFName").val(data.fName);
				$("#inputLName").val(data.lName);
				$("#inputDept").val(data.dept);
				tID = data.teacher.id;
			});
		});
	</script>
	...
	<script>
		...
		function optionMaker(data) {
			...
			for (i = 0; i < data.length; i++) {
				var selected = (data[i].id == tID) ? "selected" : "";
				o += "<option value=\"" + data[i].id + "\"" + selected + ">"
